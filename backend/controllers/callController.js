  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        const dispose = String(getVal(['dispose', 'disposition', 'final dispose', 'final_dispose', 'final disposition', 'final_disposition']) || getVal(['result', 'call result', 'callresult', 'resolution', 'status', 'call status', 'terminating reason', 'disconnect reason', 'agent status']) || '').trim();
        ...
        const duration = String(getVal(['duration', 'talktime', 'talk time', 'call duration', 'callduration', 'length']) || getVal(['call time', 'calltime']) || '').trim();
